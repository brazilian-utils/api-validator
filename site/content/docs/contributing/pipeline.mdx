...
The [Brazilian Utils](https://github.com/brazilian-utils) libraries follow one contract in [doc](https://github.com/brazilian-utils/doc). Three workflows in that repository keep the contract, the libraries and this site in step:
...
It runs the doc Action, which checks the library against the contract. The check fails only on a regression or on public API that is not in the contract. The job summary lists what is still missing.
...
| --- | --- | --- | --- |
| `LIBS_TOKEN` | secret | doc | A fine-grained token or a GitHub App, with write access to Issues, Contents and Pull requests on the library repositories. Never give it the Workflows permission. It turns on the issues and the suite PRs. |
| `PUBLISH_SITE` | variable | doc | `true` turns on the deploys to GitHub Pages from main. The Pages source must be GitHub Actions. |
| `SITE_URL` | variable | doc | The public address of the site, with its path. Set it for a custom domain. |
| `DOC_DISPATCH_TOKEN` | secret | each library | A token that can send `repository_dispatch` to the doc repository (Contents: write). Without it, a release reaches the site at the next nightly run. |
