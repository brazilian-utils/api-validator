...
As bibliotecas do [Brazilian Utils](https://github.com/brazilian-utils) seguem um contrato só, no [api-validator](https://github.com/brazilian-utils/api-validator). Três workflows desse repositório mantêm o contrato, as bibliotecas e este site em sincronia:
...
Ele roda a Action do api-validator, que verifica a biblioteca contra o contrato. A verificação só falha numa regressão ou numa API pública que não está no contrato. O resumo do job lista o que ainda falta.
...
| --- | --- | --- | --- |
| `LIBS_TOKEN` | segredo | api-validator | Um token fine-grained ou um GitHub App, com escrita em Issues, Contents e Pull requests nos repositórios das bibliotecas. Nunca dê a ele a permissão Workflows. Ele liga as issues e os PRs da suíte. |
| `PUBLISH_SITE` | variável | api-validator | `true` liga os deploys no GitHub Pages a partir de main. A fonte do Pages precisa ser GitHub Actions. |
| `SITE_URL` | variável | api-validator | O endereço público do site, com o caminho. Defina para um domínio próprio. |
| `API_VALIDATOR_DISPATCH_TOKEN` | segredo | cada biblioteca | Um token que pode enviar `repository_dispatch` para o api-validator (Contents: write). Sem ele, uma release chega ao site na próxima execução noturna. |
