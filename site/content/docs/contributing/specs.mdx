...
Do not rename a function. A new name breaks every library. To change only the title on the page, add `label` with `{ "en", "pt-BR" }`. [docs/contract.md](https://github.com/brazilian-utils/doc/blob/main/docs/contract.md) describes every field.
...
3. Run `npm run lint`. It checks the contract, the schemas and the format. `npm run doc fmt` fixes the format.

4. Run `npm run doc sync`, then `npm run check -- --tests` to see which libraries already pass the new cases.
