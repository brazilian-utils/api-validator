...
Do not rename a function. A new name breaks every library. To change only the title on the page, add `label` with `{ "en", "pt-BR" }`. [docs/contract.md](https://github.com/brazilian-utils/docs/blob/main/docs/contract.md) describes every field.
...
## Cases from a library's own tests

A library that already tests a function has cases the contract can take: `npm run docs mine-tests -- -l javascript` reads the test files of the JavaScript library and takes every assertion of the form `expect(fn(literals)).toBe(literal)` on a function the contract knows. It writes them to `contract/_proposals/` for review, and with `--apply` it appends them to the contract, each with a `note` that names the test they came from. It leaves alone what the contract cannot hold: computed inputs or outputs, inputs of a type the signature does not admit, a forced random value, and inputs the contract already has. Then run `npm run check -- --tests` on every library: a case that only the source library passes is a divergence to settle before the merge.
