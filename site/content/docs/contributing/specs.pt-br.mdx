...
O contrato no [docs](https://github.com/brazilian-utils/docs) define cada utilitário: uma pasta por utilitário (o contrato chama de domínio), com um `contract.json`. Cada arquivo lista as funções, as assinaturas e os casos de teste que todas as bibliotecas rodam. Este site gera as páginas a partir do contrato. Quando uma mudança no contrato entra na main, cada biblioteca que não tem a função nova recebe uma issue.
...
Não renomeie uma função. Um nome novo quebra todas as bibliotecas. Para mudar só o título na página, adicione `label` com `{ "en", "pt-BR" }`. O [docs/contract.md](https://github.com/brazilian-utils/docs/blob/main/docs/contract.md) descreve todos os campos.
...
## Casos dos testes da própria biblioteca

Uma biblioteca que já testa uma função tem casos que o contrato pode receber: `npm run docs mine-tests -- -l javascript` lê os arquivos de teste da biblioteca JavaScript e pega toda asserção da forma `expect(fn(literais)).toBe(literal)` sobre uma função que o contrato conhece. Ele escreve os casos em `contract/_proposals/` para revisão, e com `--apply` acrescenta ao contrato, cada um com uma `note` que diz de qual teste veio. Ele deixa de lado o que o contrato não comporta: entradas ou saídas calculadas, entradas de um tipo que a assinatura não admite, um valor aleatório forçado e entradas que o contrato já tem. Depois rode `npm run check -- --tests` em todas as bibliotecas: um caso que só a biblioteca de origem passa é uma divergência para resolver antes do merge.
