...
If a library has both, its usage files win for the functions that they document. `api-validator usage --lib <lib> --materialize` converts a reference page into usage files. Use it to move a library to usage files, or to update the site's offline copy.
...
A library without `docs/usage/` uses the copies in api-validator's `site/fixtures/usage/<lib>/`. These copies have the same format, so you can copy them into the library as they are.
