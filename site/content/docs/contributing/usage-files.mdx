...
- **The heading is the function name**: `isValid`, `format`, `parse`, `generate`, `getInfo`. The site also accepts `get-info`, the default title (`Validate`) and the old name `validate`. `api-validator usage` reports a heading that matches no function, and the build skips it.
