...
- **O título é o nome da função**: `isValid`, `format`, `parse`, `generate`, `getInfo`. O site também aceita `get-info`, o título padrão (`Validate`) e o nome antigo `validate`. O `doc usage` aponta um título que não corresponde a nenhuma função, e o build o ignora.
...
Se uma biblioteca tem os dois, os arquivos de uso têm prioridade nas funções que eles documentam. `doc usage --lib <lib> --materialize` converte uma página de referência em arquivos de uso. Use o comando para passar uma biblioteca para arquivos de uso, ou para atualizar a cópia offline do site.
...
Sem `--scaffold`, o `doc usage` mostra o que está documentado, o que falta e o que está errado. Com `--strict`, o comando falha quando falta algo ou algo está errado. Uma biblioteca pode usar isso na CI.
...
Uma biblioteca sem `docs/usage/` usa as cópias em `site/fixtures/usage/<lib>/` do repositório doc. Essas cópias têm o mesmo formato, então você pode copiá-las para a biblioteca do jeito que estão.
...
1. Copie `site/fixtures/usage/<lib>/` do repositório doc para `docs/usage/` na biblioteca, ou rode o comando de [Gere os primeiros exemplos](#gere-os-primeiros-exemplos).
