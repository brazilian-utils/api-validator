...
- **O título é o nome da função**: `isValid`, `format`, `parse`, `generate`, `getInfo`. O site também aceita `get-info`, o título padrão (`Validate`) e o nome antigo `validate`. O `api-validator usage` aponta um título que não corresponde a nenhuma função, e o build o ignora.
