...
export function LangIcon({ lib, className = 'size-4' }: { lib: string; className?: string }) {
  ...
  if (!icon) return null;
  // Rust's brand color is black: use the text color so it shows in the dark theme.
  const color = icon.hex === '000000' ? 'currentColor' : `#${icon.hex}`;
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className={`shrink-0 ${className}`} fill={color}>
      <path d={icon.path} />
    </svg>
  );
}
