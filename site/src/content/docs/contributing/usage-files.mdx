...
Each utility page shows the spec, the signature and the test cases from the contract in [api-validator](https://github.com/brazilian-utils/api-validator). The code examples in the tabs come from the libraries. Each library writes its own examples in its own repository. Whoever changes the API updates the examples in the same pull request. This site only reads them.

A library can publish its examples in three ways. It can use one, two or all three:
...
Write one file for each utility. Give the file the name of the page (`cpf.md`, `license-plate.md`) or of the contract domain (`licensePlate.md`). In the file, write one `##` heading for each function. The heading is the **function name in the contract**, which is a key of `functions` in `contract/<domain>.json`:
...
- **The heading is the function name**: `isValid`, `format`, `parse`, `removeSymbols`, `generate`, `getInfo`. The site also accepts `remove-symbols`, the default title (`Validate`) and the old name `validate`. `api-validator usage` reports a heading that matches no function, and the build skips it.
...
If a library has both, its usage files win for the functions that they document. `api-validator usage --lib <lib> --materialize` writes a reference page as usage files. Use it to move a library to usage files, or to update the site's offline copy.
...
In a guide, mark each example with these tags. Put each tag on its own line. docsify shows the same markup, so a library can use the same files for its own site.
...
The command does not change sections that exist. It adds the new sections at the end of the file. Each result in a comment is the value that the contract expects, for a case that the library passes. The examples are correct when the command writes them. You can edit them after that.

Without `--scaffold`, `api-validator usage` shows what is documented, what is missing and what is wrong. With `--strict`, the command fails when something is missing or wrong. A library can use it in its CI.
...
A library without `docs/usage/` uses the copies in `site/fixtures/usage/<lib>/` of api-validator. These copies have the same format, so you can copy them into the library as they are.
...
1. Copy `site/fixtures/usage/<lib>/` from api-validator to `docs/usage/` in the library, or run the command in [Generate the first examples](#generate-the-first-examples).
